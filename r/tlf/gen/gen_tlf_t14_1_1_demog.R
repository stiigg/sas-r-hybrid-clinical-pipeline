...
source("r/tlf/utils/manifest_helpers.R")
source("r/tlf/utils/tlf_log.R")
source("r/tlf/utils/safe_io.R")
source("r/tlf/utils/file_paths.R")
...
generate_tlf <- function(tlf_id, config, dry_run = FALSE) {
  ...
  manifest_path <- file.path(config$paths$output_dir %||% "r/tlf/output", "manifest.csv")
