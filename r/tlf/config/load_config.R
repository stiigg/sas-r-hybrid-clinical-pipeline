...
#'   "r/tlf/config/tlf_config.yml".
#' @return A named list representing the configuration.
load_tlf_config <- function(config_path = "r/tlf/config/tlf_config.yml") {
  ...
  options(
    tlf.config = config,
    tlf.paths = paths,
    tlf.output_dir = paths$output_dir %||% "r/tlf/output",
    tlf.log_dir = paths$log_dir %||% "logs",
    tlf.qc_report_dir = paths$qc_reports %||% file.path(getOption("tlf.output_dir"), "qc"),
...
load_tlf_shell_map <- function(manifest_path = "r/tlf/config/tlf_shell_map.csv") {
...
#' @return The full path to the script within r/tlf.
...
get_tlf_output_path <- function(filename) {
  file.path(getOption("tlf.output_dir", "r/tlf/output"), filename)
}
